	...
	describe '.send()', ->
		...
		describe 'given an interceptor', ->
			...
			describe 'with a request hook', ->
				...
				it 'should let the interceptor change a request before it is sent', ->
				...
				it 'should let the interceptor asynchronously change a request before it is sent', ->
					request.interceptors[0].request = (request) ->
				...
				it 'should let the interceptor stop a request', ->
			...
			describe 'with a requestError hook', ->
				...
				it 'should call requestError only in the subsequent hook, if there are errors', ->
					request.interceptors[0] =
